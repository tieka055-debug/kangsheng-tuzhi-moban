...
- 公差栏有两种明确的 v2 模式：默认搬运各型号原始矢量表；仅在 `approved_tolerance_reflow` 有逐型号原图字段映射、原图裁区指纹及认可成品哈希时，使用已认可的英文疏排栏。该模式只替换公差栏标题、排字和内网格；源公差组仍纳入覆盖及原始像素差报告，实际输出逐项核对，其他工程区域门槛不变。投影符号仍从该型号原图搬到右下格。具体边界见 [Manifest v2](references/manifest.md)。
- 英文模式只支持原表确认恰有四档且表内无额外公差条件的结构；数值、档位、单位、正负号、小数位必须来自该型号原图，不使用任何全型号默认值。其他技术组中的附加公差条件须独立定位、保持原矢量并逐组 QA；表内额外条件、角度档位或未核实内容应停止。
